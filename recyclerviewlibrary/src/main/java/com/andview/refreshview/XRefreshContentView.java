    ...
    private void setRecyclerViewScrollListener() {
        ...
        mOnScrollListener = new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                //dy垂直移动为负数的情况下，向上移动---显示
                //dy垂直移动为正数的情况下，向下移动---不显示
                //两次滑动的距离之和大于100才进行数据统计
                if(dy>=0){
                    accumulation=accumulation+dy;//a+=b
                }else{
                    accumulationfu=accumulationfu+dy;
                }

//                LogUtilss.i(dy+"=oldy+dy="+(oldy+dy)+"=accumulation="+accumulation+"=accumulationfu="+accumulationfu);

                if(oldy+dy>120||oldy+dy<-120||accumulation>120||accumulationfu<-120){
                    if(oldy+dy>120||oldy+dy<-120){
                        doOnBorderListener(oldy+dy >= 0 ? false : true);
                    }else if(accumulation>120||accumulationfu<-120){
                        doOnBorderListener(accumulation >= 120 ? false : true);
                    }
                    accumulation=0;//每次显示完成就重置
                    accumulationfu=0;
                }
...
//                    LogUtilss.i("这里哦哦111");
    ...
    private void doSilenceLoadMore(BaseRecyclerAdapter adapter, RecyclerView.LayoutManager layoutManager) {
        if (!mIsLoadingMore && isOnRecyclerViewBottom() && !hasLoadCompleted()) {
            if (mRefreshViewListener != null) {
                LogUtilss.i("mIsLoadingMore = true11111111111");
                mIsLoadingMore = true;
                mRefreshViewListener.onLoadMore(true);
            }
        }
...
//        LogUtilss.i("!mIsLoadingMore="+!mIsLoadingMore+"=isOnRecyclerViewBottom()="+isOnRecyclerViewBottom()+"=mHideFooter="+mHideFooter);
...
//        LogUtilss.i("!isFooterEnable()="+!isFooterEnable()+"=mIsLoadingMore="+
//                mIsLoadingMore+"=mFooterCallBack="+(mFooterCallBack==null?true:false)+"=!hasLoadCompleted()="+!hasLoadCompleted());
        ...
        if (!hasLoadCompleted()) {
            ...
            LogUtilss.i("mIsLoadingMore = true2222222");
            mIsLoadingMore = true;
    ...
    public void notifyRecyclerViewLoadMore() {
        if (!mIsLoadingMore) {
            if (!hasLoadCompleted()) {
                ...
                LogUtilss.i("mIsLoadingMore = true333333");
                mIsLoadingMore = true;
                previousTotal = mTotalItemCount;
                mFooterCallBack.onStateRefreshing();
                setState(XRefreshViewState.STATE_LOADING);
...
//        LogUtilss.i(isRecyclerView()+"=isRecyclerView()hideFooterhideFooter="+hideFooter);
        mIsLoadingMore = false;
//        mTotalItemCount = 0;
        if (mFooterCallBack != null) {
...
//        mTotalItemCount = 0;
    ...
    private void dealRecyclerViewNotFullScreen() {
        ...
        if (onRecyclerViewTop() && !Utils.isRecyclerViewFullscreen(recyclerView) && child instanceof RecyclerView && mFooterCallBack != null && isFooterEnable()) {
            mFooterCallBack.onStateReady();
//            LogUtilss.i("这里哦哦333333333333333333333111");
            mFooterCallBack.callWhenNotAutoLoadMore(mParent);
            if (!mFooterCallBack.isShowing()) {
                mFooterCallBack.show(true);
            }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (mSilenceLoadMore) {
            if (mRefreshViewListener != null && !hasLoadCompleted() && !mIsLoadingMore && mTotalItemCount - 1 <= view.getLastVisiblePosition() + mPreLoadCount) {
                mRefreshViewListener.onLoadMore(true);
                LogUtilss.i("mIsLoadingMore = true4444444444");
                mIsLoadingMore = true;
            }
        } else if (mContainer != null && !hasLoadCompleted()
     ...
     * 商品头标题切换
     * */
    OnXScrollListener onXScrollListener;
    private void doOnBorderListener(boolean isScrollToTop) {
        if(isTop()&&onXScrollListener!=null){
            //在顶端
            onXScrollListener.onTop();
        }else if(onXScrollListener!=null){
            //滚动时的监听
            onXScrollListener.onScroll(isScrollToTop);
        }
    }

    /**
     * 定义接口比定义抽象类好，有用时进行实现，不需要监听时就不实现代码
     * */
    public static interface OnXScrollListener{
         ...
         *
         * @param isScrollToTop*/
        public void onScroll(boolean isScrollToTop);
    }

    public void setOnXScrollListener(final OnXScrollListener onXScrollListener) {
